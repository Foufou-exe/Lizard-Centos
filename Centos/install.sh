...
Script_PATH1="~/LizardFS-Centos/Centos/Master.sh"
Script_PATH2="~/LizardFS-Centos/Centos/Chunk.sh"
Script_PATH3="~/LizardFS-Centos/Centos/Chunk_MetaLogger.sh"
Script_PATH4="~/LizardFS-Centos/Centos/Shadow.sh"
Script_PATH5="~/LizardFS-Centos/Centos/Client.sh"
...
function pre-requis {   #   Pre-requisite function: It will aim at gathering the necessary installation
    ...
    sleep 1 # Wait a second
    yum install -y lsb-release wget gnupg apt-transport-https dpkg-dev gzip epel-release yum-utils # Install the prerequisites
    curl http://packages.lizardfs.com/yum/el7/lizardfs.repo > /etc/yum.repos.d/lizardfs.repo #  Curl : We put the links of the lizards packages of theyum.repo
    yum update # Update the System Package
    yumdownloader --source lizardfs # Download the System Package lizardfs
    sleep 1 # Wait a second
